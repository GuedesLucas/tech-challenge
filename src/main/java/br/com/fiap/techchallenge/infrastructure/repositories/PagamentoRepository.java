...
public interface PagamentoRepository  extends JpaRepository<Pagamento, Long> {
    ...
    Pagamento findByPedidoAndGatewayAndStatus(@Param("pedido") Pedido pedido, @Param("gatewayPagamentoID") String gatewayPagamentoID, @Param("status") StatusPagamentoEnum status);

    @Query("SELECT p FROM Pagamento p WHERE p.gateway.gatewayPagamentoID = :gatewayPagamentoID AND p.status = :status")
    Pagamento findByGatewayAndStatus(@Param("gatewayPagamentoID") String gatewayPagamentoID, @Param("status") StatusPagamentoEnum status);
}
